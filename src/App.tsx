        ...
        {todos.map(todo => (
          <li key={todo.id} className="flex items-center mb-2">
            <input
              ...
              className="mr-2"
            />
            <span className={`text-sm ${todo.completed ? 'line-through text-gray-500' : 'text-gray-900'}`}>{todo.text}</span>
            <button
              onClick={() => handleDeleteTodo(todo.id)}
              className="ml-auto p-1 text-sm text-red-500 hover:text-red-700 focus:outline-none focus:ring-2 focus:ring-red-600"
            >
              Delete
            </button>
          </li>
